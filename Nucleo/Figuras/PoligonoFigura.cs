...
namespace Paint_Bolaños_Flores_Venegas.Nucleo
{
    public class PoligonoFigura : Figura2D
    {
        ...
        public override void Rasterizar(ContextoRaster c)
        {
            ...
            if(Estilo.TieneRelleno&&p.Count>=3)new RellenoScanline().Rellenar(c.Buffer,p,Estilo.ColorRelleno);c.Linea=new LineaBresenham();for(int i=0;i<p.Count;i++)c.DibujarSegmento(p[i],p[(i+1)%p.Count],Estilo.ColorLinea,Estilo.Grosor);
        }
